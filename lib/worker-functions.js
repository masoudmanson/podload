(function() {
  function workerFunctions(params) {
    ...
    this.workerGetHistory = function(chatAgent, process, startTime) {
      var completeCount = 0,
        actionTimeoutInterval = {};

      process.send({
        type: CHAT_ACTIONS.GET_HISTORY,
        content: {
          type: "chatReady",
          state: "passed",
          duration: new Date().getTime() - startTime
      ...
      chatAgent.getThreads({
        ...
        if (!result.hasError) {
          do {
            var getHistoryStartTime = new Date().getTime();
            ...
            actionTimeoutInterval[iterationCount] = setTimeout(() => {
              process.send({
                ...
                content: {
                  type: "getHistory",
                  state: "timeout",
                  duration: new Date().getTime() - getHistoryStartTime
                }
              });
            ...
            if (result.result.threads.length > 0) {
              ...
              params.nodelog && console.log(`    ☰ Worker ${process.pid} \tGet History Request\t ${new Date()}`);
